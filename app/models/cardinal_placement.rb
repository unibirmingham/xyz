class CardinalPlacement < Rule
	...
	def self.all
		url = build_url_for('/action=Query&databasematch=activated&print=all&maxresults=5000&fieldtext=MATCH{1}:QMSTYPE&Text=*')
	...
	def self.find(id)
		url = build_url_for "/action=Query&databasematch=activated&print=all&maxresults=5000&fieldtext=MATCH{1}:QMSTYPE&MATCHID=#{id}"
		data = query_idol_with(url)
